...
function displayUkSchedule(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    scheduleItem.innerHTML = '<div class="show-title">' + item.show.name + '</div>' + '<div class="time">' + new Date('1970-01-01T' + item.airtime + 'Z').toLocaleTimeString([], {
      hour: '2-digit',
      minute: '2-digit'
    }) + '</div>' + '<div class="channel">' + (item.show.network ? item.show.network.name : 'Unknown') + '</div>';
